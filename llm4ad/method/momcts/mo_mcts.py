...
class MCTS:
    def __init__(self, root_answer: Any, num_objectives: int, bounds: List[float, float], exploration_constant_0: float = 0.1, alpha: float = 0.5):
    ...
    def _calculate_multi_objective_ucb(self, child: MCTSNode, parent_visits: int) -> List[float]:
        ...
        for i in range(self.num_objectives):
            ...
            if child.visits > 0:
                normalized_vals = [
                    (r[i] - self.lower_bound) / (self.upper_bound - self.lower_bound)
