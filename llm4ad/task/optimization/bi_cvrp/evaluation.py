...
def evaluate(instance_data, n_instance, capacity, evaluate_func: callable):
    ...
    for i, (coords, demand, distance_matrix_1, distance_matrix_2) in enumerate(instance_data):
        start = time.time()
        init_solutions = [random_solution(len(demand) - 1, capacity, demand) for _ in range(10)]
        archive = [
            (s, evaluate_solution(s, distance_matrix_1, distance_matrix_2))
            for s in init_solutions
            if is_feasible_solution(s, demand, capacity)
        ]
        
        for _ in range(2000):
            s_prime = evaluate_func(archive, coords, demand, distance_matrix_1, distance_matrix_2, capacity)
            if not is_feasible_solution(s_prime, demand, capacity):
                continue
            f_prime = evaluate_solution(s_prime, distance_matrix_1, distance_matrix_2)
        ...
        # Compute mean of the non-dominated set for this instance
        mean_obj = np.mean(objs, axis=0)
        obj_1[i], obj_2[i] = mean_obj[0], mean_obj[1]
    
    # Return average of both raw objectives (not normalized or negated)
